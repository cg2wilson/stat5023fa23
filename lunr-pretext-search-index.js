...
},
{
  "id": "lec-19-sec-1",
  "level": "1",
  "url": "lec-19-sec-1.html",
  "type": "Section",
  "number": "11.1",
  "title": "Randomized Complete Block Design",
  "body": " Randomized Complete Block Design   Randomized Complete Block Design  In a randomized complete block design, we have the following terms:   Experimental Unit: The object that receives the treatment level    Completely Randomized Design: Units are randomly assigned to each treatment level. Here, we implicitly treat each experimental unit as homogeneous.   In some situations, there may exist additional variability from other sources. Also, we may be required to use the paired t-test to remove any potential heterogeneity.  One example situation may be: Suppose we study the effect of three different fertilizers on corn yields with fifteen 1-acre plots. Assume there exists a gradien tin the plots which impacts yields; unless we take the gradient into account, our response (yields) would be affected by both fertilizers and the gradient. So, three fertilizers need to be randomly assigned within each strip.  In RCBD, treatment levels may be group or \"blocked\" to account for unwanted variability. The blocks each consist of homogeneous experimental units with a random assignment of treatment levels within each block. This means we have the table below. Let be an observation in the th block receiving the th treatment level.     Block 1  Block 2   Block    Level 1        Level 2               Level        Note that in each block, there are treatment levels, so there are observations.  The design above is a RCBD with blocks. The treatment structure is one-way with levels. We have the following statistical model associated with the table: where and , is the observation in the th block receiving the th treatment level, is the overall mean (a fixed constant), is the th treatment level effect (a fixed constant), is the th treatment block effect such that , and is a random error associated with such that   This gives us the following ANOVA table for RCBD:    Source  df  EMS  Test Statistic    Block       Treatment      Error      Total     The null hypothesis being tested is   Here, note the following: randomization has been applied only to treatments within blocks; we assume no interaction between treatments and blocks; we are not interested in testing a hypothesis about the blocking factor.  Under RCBD, we have the standard error estimates of and : Under these estimated standard errors, we can perform inference as before. For a contrast such that , we have We wish to test the hypothesis versus with under . Thus, reject if under significance level .  For multiple comparison, we have    Method  Cutoff  and Rejection Decision    Fisher   if    Tukey   if    Scheffe    if     We have the following ANOVA table for RCBD with blocks with an factorial (fixed) treatment structure:    Source  df  EMS      Block      A         B         AB     no interaction     Error      Total        An education specialist wants to compare the average difference among the three different vocabulary tests that are available for children. They select 8 children and administer all three tests, with results given below.     Child  Test Means     1  2  3  4  5  6  7  8     Test A  18  24  30  12  26  15  29  20  21.75    Test B  24  25  30  15  23  15  33  19  23.00    Test C  17  21  26  11  22  12  21  18  18.50    This results in the following ANOVA table:     df  SS  MS    Test  2  86.333  43.167    Child  7  719.85  102.84    Error  14  61.755  4.411      We test by using the -statistic under . Under , there is significant evidence that at least one vocabulary test has a different mean score; so, we do a pairwise comparison. Under , we have Tukey's HSD give which results in so we conclude that there is no significant average difference between tests A and B, while the other pairs show significant differences.     "
},
{
  "id": "example-14",
  "level": "2",
  "url": "lec-19-sec-1.html#example-14",
  "type": "Example",
  "number": "11.1.1",
  "title": "",
  "body": "  An education specialist wants to compare the average difference among the three different vocabulary tests that are available for children. They select 8 children and administer all three tests, with results given below.     Child  Test Means     1  2  3  4  5  6  7  8     Test A  18  24  30  12  26  15  29  20  21.75    Test B  24  25  30  15  23  15  33  19  23.00    Test C  17  21  26  11  22  12  21  18  18.50    This results in the following ANOVA table:     df  SS  MS    Test  2  86.333  43.167    Child  7  719.85  102.84    Error  14  61.755  4.411      We test by using the -statistic under . Under , there is significant evidence that at least one vocabulary test has a different mean score; so, we do a pairwise comparison. Under , we have Tukey's HSD give which results in so we conclude that there is no significant average difference between tests A and B, while the other pairs show significant differences.   "
},
{
  "id": "lec-20-sec-1",
  "level": "1",
  "url": "lec-20-sec-1.html",
  "type": "Section",
  "number": "12.1",
  "title": "Latin Square Design",
  "body": " Latin Square Design   Latin Square Design  Recall that in RCBD, we had one factor chosen to be blocked in order to control outside variability. A Latin Square design chooses to block two factors. The two sources of variability are represented as rows and columns; each treatment level occurs once and only once in each row and column. This procedure requires that there be exactly rows and columns, for the treatments.  As a quick example, consider this situation: A consumer organization wants to evaluate three major brands, , of room air cleaners. In order to make the ratings as realistic as possible, they choose to evaluate the air cleaners outside the testing laboratory in residential homes. To control for variations due to the differening air qualities in the homes and due to the time-of-year characteristics of external air pollution, they use a cleaner of each brand in each of three homes and to run the tests at three different months. This would result in the following square:     Home 1  Home 2  Home 3    Month 1       Month 2       Month 3       For Latin Squares, proper random assignment of treatment matters. We may randomly assign some labels to columns in Row 1, then assign them to remaining rows so that each label appears only once in each row and column. Then, we randonmly assign the treatment levels to each label.  The statistical effects model is given by for with    being the observation at row and column with treatment level      the overall mean     the effect of row such that      the effect of row such that      is a fixed (non-random) effect of treatment level      is a random error.     We have the following source of variation in a Latin Square Design:    Source  df  EMS      Row      Column      Treatment        Error      Total     Note the following:   The treatment mean is given by          The standard error estimate of is given by    Also, can be estimated by . This means that             A CI for is given by      GIven the above estimated standard errors, we can make inferences using contrasts and multiple comparisons. Let be a contrast of interest. Then,   A CI for is given by Note that is and is given by     We may hypothesis test vs. using a test or test as before: or where . Note that we reject if or      When two sources of outside variability can be well-identified, the Latin square is an effective design. We only restrict that the number of treatments be equal to the number of rows and columns. Small designs generally have too few replications for a desirable level of precision, however.    The study of home air cleaners as a Latin square design using randomly chosen five homes and five months as blocking variables is given below; the response variable is the CADR value obtained from a room air cleaner in a given home during a given month.        Month  1  2  3  4  5  Month Mean  Brand Mean    1       159.6  182.2    2       181.8  139.8    3       198.0  165.6    4       173.4  229.0    5       279.0  275.2    Home mean  196.6  206.4  211.6  189.6  187.6       Write an appropriate statistical model for this experimental situation    Test whether the five brands show any average difference or not under . Assume and     Under , test whether the average CADR of Brands 4 and 5 using            , where and are random effects for the row and column, respectively, and is the effect for the treatment.     , which results in rejection of the null (double check).             Latin Square Design with Multiple Squares  Consider a Latin square:    Source  df    Row  1    Column  1    Treatment  1    Error  0    Total  3    Here, there are no degrees of freedom for error; this means that no test of significance can be done. For this reason, more than one Latin square can be used in an experiment. The larger df means that the have a better estimate of the error variance.  For many Latin squares, the sources of variation can generally be partitioned as follows:    Source  df    Square     Row within Square     Column within Square     Treatment     Error     Total     As an example, assume that there are two lab technicians to carry out an experiment. Each of the technicians has different \"luck\" in carrying out the experiment.     Technician 1     Oven 1  Oven 2  Oven 3  Oven 4    Day 1  A  B  D  C    Day 2  D  C  A  B    Day 3  C  A  B  D    Day 4  B  D  C  A       Technician 2     Oven 5  Oven 6  Oven 7  Oven 8    Day 5  B  D  A  C    Day 6  A  C  B  D    Day 7  D  A  C  B    Day 8  C  B  D  A    What are the sources of variation in the experiment?   "
},
{
  "id": "example-15",
  "level": "2",
  "url": "lec-20-sec-1.html#example-15",
  "type": "Example",
  "number": "12.1.1",
  "title": "",
  "body": "  The study of home air cleaners as a Latin square design using randomly chosen five homes and five months as blocking variables is given below; the response variable is the CADR value obtained from a room air cleaner in a given home during a given month.        Month  1  2  3  4  5  Month Mean  Brand Mean    1       159.6  182.2    2       181.8  139.8    3       198.0  165.6    4       173.4  229.0    5       279.0  275.2    Home mean  196.6  206.4  211.6  189.6  187.6       Write an appropriate statistical model for this experimental situation    Test whether the five brands show any average difference or not under . Assume and     Under , test whether the average CADR of Brands 4 and 5 using            , where and are random effects for the row and column, respectively, and is the effect for the treatment.     , which results in rejection of the null (double check).          "
},
{
  "id": "lec-21-sec-1",
  "level": "1",
  "url": "lec-21-sec-1.html",
  "type": "Section",
  "number": "13.1",
  "title": "Nested Factor and Subsampling",
  "body": " Nested Factor and Subsampling   Nested Factor and Subsampling  As a motivating example, consider the situation where a pharmaceutical company conducts test to determine stability of its product in two manufacturing sites. At each site, a random sample of three batches of the product was obtained, and additional random samples of 10 different tablets were obtained from each batch.  Note that in this situation, we are not considering factorial design; we have three batches at Site 1 which are distinct from those at Site 2.   Nested Factor\/Crossed Factor   Factor B is said to be nested within levels of A if the physical properties of the levels of Factor B vary depending on which level of Factor A it is associated with. Two factors A and B are crossed if the physical properties of the levels of Factor B are identical for all levels of Factor A.  In other words, a nested factor is one whose levels are unique to each level of a second factor.    In the previous example, batch is nested within site, and tablet is nested within batch (hence site).  Suppose we study whether or not there exists any difference on average response in patients having the same therapy among hospitals. A CRD with one-way treatment structure would have us select patients at each of randomly chosen hospitals. The hospital has randomly selected treatment levels, which are normally distributed as ; patients are randomly chosen experimental units unique to each hospital, normally distributed as . Here, each experimental unit can be nested within a treatment level. Error comes from variability from patients within the hospital, so we have    Source  df  EMS    Treatment      Error      Total  ts-1    Suppose that we have two drugs to be compared at the same, randomly chosen 10 hospitals. We have a two-way factorial treatment structure with random effect for hospital from and fixed effects for drugs. Select patients for Drug 1 at each hospital and patients for Drug 2 at each hospital. There exists interaction between the hospital and the drug, from . The error then is due to patients within Drug and Hospital, taken from . This gives    Source  df  EMS    Drug  1     Hospital  9     Drug Hospital  9     Error      Total     Now suppose 10 hospitals are randomly selected to test Drug 1 and another 10 hospitals are randomly selected to test Drug 2. A random sample of patients is chosen in each hospital. In this case, Hospital is nested within Drug; this gives different df from the factorial case and . It's not possible to model the interaction because observations from all Drug and Hospital combinations are not available. The error is due to patients nested within Hospital and Drug, so     Source  df  EMS    Drug      Hospital (within Drug)      Error      Total     Here, we have versus with the test statistic and versus , with the test statistic   A subsample is a subvidision of an experimental unit, i.e. it is nested within an experimental unit. In the previous example, the treatment is the drug, the experimental unit is the hospital, and the subsampling unit is the patient.  As another example, if we have a treatment of toxic levels, our experimental unit could be an aquarium, with a subsampling unit of fish.  A statistical effects model with a treatment structure of one-way with levels, as a CRD with replications and subsamples can be given as for , , and , where    is the observation on subsample of experimental unit in treatment level      is the fixed overall mean     is a fixed effect of treatment level      is random error due to the th experimental unit within treatment level .      is the random error due to the th subsample in experimental unit within treatment level .      In general, we can partition the sources of variation of the model as follows:    Source  df  EMS    Treatment      Exp. Error      Subsampling Error      Total     Here, is the number of experimental units, and is the total number of observations. The exp. error term is the error due to the experimental unit within each treatment, and subsampling error is due to the subsample in the experimental unit within each treatment.   "
},
{
  "id": "definition-3",
  "level": "2",
  "url": "lec-21-sec-1.html#definition-3",
  "type": "Definition",
  "number": "13.1.1",
  "title": "Nested Factor\/Crossed Factor.",
  "body": " Nested Factor\/Crossed Factor   Factor B is said to be nested within levels of A if the physical properties of the levels of Factor B vary depending on which level of Factor A it is associated with. Two factors A and B are crossed if the physical properties of the levels of Factor B are identical for all levels of Factor A.  In other words, a nested factor is one whose levels are unique to each level of a second factor.   "
},
{
  "id": "p-411",
  "level": "2",
  "url": "lec-21-sec-1.html#p-411",
  "type": "Paragraph (with a defined term)",
  "number": "",
  "title": "",
  "body": "subsample "
},
{
  "id": "lec-22-sec-1",
  "level": "1",
  "url": "lec-22-sec-1.html",
  "type": "Section",
  "number": "14.1",
  "title": "Split-Plot Design",
  "body": " Split-Plot Design   Split-Plot Design  Consider a two-way factorial treatment structure. A random assignment of one factor would be difficult to apply to the experimental units of the other factor; for example, if we consider the yields of three different varieties of cottons compared under two different irrigation systems with two observations at each treatment combination, we see that the irrigation systems must apply the water to large sections of land and that a CRD might be time-consuming.  Each irrigation system would be applied to two larger \"whole plots\" and the three varieties of cottons would be planted in three \"subplots\" within each whole plot:   A split-plot design is applicable to factorial treatment structure only with two different sizes of experimental units: whole-plot experimental unit or main unit; subplot experimental unit or subunit. Split-plot design involves more than one randomization when assigning treatments to experimental units. Consider Factor A in a CRD with replications per level. The levels of factor A are applied to a whole plot, which is then subdivided into subplots to which one of the levels of Factor B are assigned, as in the image below.   For experimental error, we consider the variance among things treated alike. In split-plot design, the two factors A and B have two different sizes of experimental units. This means there are two sources of error variance considered: variability between whole plots within each level of factor A; and variability among subplots.  This makes the treatment structure factorial with factors A and B as whole-plot and subplot treatment, respectively. The design structure is split-plot in a CRD with replications.  A statistical effects model for an factorial treatment structure in a completely randomized split-plot design is given by where is focused on the whole-plot, while the trailing three terms focus on the subplot. Here, , , and . Specifically, we also have    : the th response of      : the fixed overall mean     : the fixed main effects of , respectively     : the fixed interaction effects of      : random error due to the th experimental unit within , with      : random error due to the th experimental unit in such that .   Note that the error terms are independent of each other. This gives the table below:     Source  df  MS  EMS    Whole Plot Analysis    MSA      WP Error   MSWPE     Subplot Analysis    MSB        MSAB      SP Error   MSSPE      Total     For interaction effects, we reject when If the interaction is insignificant, we proceed to the main effects as follows:   Reject if     Reject if    In the event that the main effects are significant, we can compare the average responses to the two levels of each factor:     estimates  standard error estimates  df for statistic                The above standard error formulae can be generalized for any contrast such that or by replacing 2 with or .  Note that for , we have with where df is described in the table. Similarly, when testing the terms, we have   We can perform pairwise comparison of levels of factors A and B in split-plot design:    Method  Cutoff  Decision    Fisher (A)   Reject if    Fisher (B)   Reject if    Tukey (A)   Reject if    Tukey (B)   Reject if    Scheffe (A)   Reject if    Scheffe (B)   Reject if    If the interaction is significant, then we compare the group means among each treatment combination. The standard error estimates and corresponding df get different depending on the types of comparison we make.     estimates  standard error estimates  df for statistic          Difference in B at the same level of A      pooled estimates  Satterthwaite df    Difference in A at the same levels of B      pooled estimates  Satterthwaite df    Difference in A at different levels of B    For pooled estimates and Satterthwaite df, we use     A study was designed to determine if additional phosphorus applied to the soil would increase the yield of soybeans. There are three major varieties of soybeans of interest (V1, V2, and V3) and four levels of phosphorus (0, 30, 60, and 90 pounds per acre). Researchers have nine plots of land available for the study. Because of the complexities of planting the soybeans on the plots of the given size, it was decided to plant a single variety of soybeans on each plot and then divide each plot into four subplots. Researchers randomly assigned a variety to one plot within each block of three plots and then randomy assigned the levels of phosphorus to the four subplots within each plot.     Identify an experimental design and an appropriate statistical model which are best consistent with the given description of the study.    Consider the following ANOVA table:    Source  df  SS  MS    Variety  2  671.81  335.90    WP Error  6  6.56  1.64    Phosphorus  3  136.12  136.12    Variety*Phosphorus  6  117.41  19.57    SP Error  18  4.08  0.23    Under , provide your complete conclusion about interaction and main effects.          We use a split plot design and the consequent model from above:     For the interaction, we have This interaction is not significant. For the main effects, we have for the variety, and for the phosphorus.        "
},
{
  "id": "example-16",
  "level": "2",
  "url": "lec-22-sec-1.html#example-16",
  "type": "Example",
  "number": "14.1.1",
  "title": "",
  "body": "  A study was designed to determine if additional phosphorus applied to the soil would increase the yield of soybeans. There are three major varieties of soybeans of interest (V1, V2, and V3) and four levels of phosphorus (0, 30, 60, and 90 pounds per acre). Researchers have nine plots of land available for the study. Because of the complexities of planting the soybeans on the plots of the given size, it was decided to plant a single variety of soybeans on each plot and then divide each plot into four subplots. Researchers randomly assigned a variety to one plot within each block of three plots and then randomy assigned the levels of phosphorus to the four subplots within each plot.     Identify an experimental design and an appropriate statistical model which are best consistent with the given description of the study.    Consider the following ANOVA table:    Source  df  SS  MS    Variety  2  671.81  335.90    WP Error  6  6.56  1.64    Phosphorus  3  136.12  136.12    Variety*Phosphorus  6  117.41  19.57    SP Error  18  4.08  0.23    Under , provide your complete conclusion about interaction and main effects.          We use a split plot design and the consequent model from above:     For the interaction, we have This interaction is not significant. For the main effects, we have for the variety, and for the phosphorus.      "
},
{
  "id": "lec-23-sec-1",
  "level": "1",
  "url": "lec-23-sec-1.html",
  "type": "Section",
  "number": "15.1",
  "title": "Experiments with Repeated Measures",
  "body": " Experiments with Repeated Measures   Experiments with Repeated Measures  An experiment with repeated measures is one in which each experimental unit has more than a single measurement taken over time or location. For example, if patients are randomly assigned to one of two medications, they may have a response measured every week for three weeks. Comparison of medications, weeks on medications, and the interaction between the two can be investigated. Let denote an observation at time on experimental unit of treatment level .      Time      Week 1  Week 2  Week 3    Med 1  Patient 1        Patient 2       Med 2  Patient 1        Patient 2       In general, for a one-way treatment structure with levels and for a CRD with repetitions and repeated measures, the sources of variation are given by    Source  df    Trt     Time     Trt*Time     Error     Total     Note that in the previous example, (two medicines), (two patients), and (three repetitions).  A statistical effects model with this design and treatment structure is given by for , , and with    : observation at time on experimental unit of treatment level      : fixed overall mean     : fixed effect of treatment level      : fixed effect of time      : interaction of th treatment and th time     : random error associated with . Note: it would not be likely that these are iid in this context.     So far, we have assumed observations are independent. However, in a repeated measures setup, and are likely to be correlated; in order to appropriately analyze the repeated measures, we need to account for how the observations within each subject are dependent to each other.  The solution is to use covariance (a numerical measure of linear association between two random variables). Covariance generalizes variance to multiple variables. A small covariance indicates no linear association between two random variables, while a larger one indicates a stronger association.  In the medication example above, the measurements are made on the same unit for three weeks. So, we need to consider the covariance structure between Week 1 and Week 2, Week 1 and Week 3, and Week 2 and Week 3.  The structure of covariance for the model must be chosen specifically. We have four options; let :   Variance Components (VC):   VC is also referred to as \"independence\"; specifically, this means that there is no linear relationship between different random variables    Compound Symmetry (CS):   We assume that each covariance is the same between different random variables    First-Order Auto-Regressive, AR(1):   AR(1) assumes that as measurements become further apart, extra factors of are needed (eg, Time 1 and Time 2 only need , but Time 1 and Time 3 need )    Unstructured (UN):   Unstructured assumes separate covariacne for each pair of repeated measurements.     Choosing an appropriate structure is challenging and beyond the scope of the course; SAS provides some measures of model fit such as AIC, BIC. The Null Model Likelihood Ratio Test compares VC to the other specified one by testing the hypothesis versus The outputs can guide us to choose the covariance structure.    An experiment is conducted to determine the effect of salt in the soil on the emergence of grass seeds. There are three replications of four plots, each treated to contain 0, 8, 16, and 24% salt. The percent of seeds emerging is observed 5, 8, 11, and 14 days after planting. THere are 12 separate plots; that is, the three replications within each level of salt are unrelated. Within each plot, there are four repeated measurements corresponding to days after planting.   The repeated measures can be analyzed using the following SAS code:  DATA one;  INPUT trt time expunit response;  DATALINES;  ...  PROC MIXED DATA = one PLOTS=RESIDUALPANEL;  CLASS trt time expunit;  MODEL response = trt time trt*time\/ DDFM = SATTERTH  REPEATED \/ SUBJECT = expunit(trt) TYPE = < option >  The line SUBJECT = expunit(trt) shows that each experimental unit is nested within treatment. The TYPE = < option > specifies covariance structure.  Based on the SAS output (salt_output(1).pdf), we see the following (for VC):   The \"Estimated R Matrix\" identifies the relationship between the four time roles; this indicates that ; this is confirmed in the \"Covariance Parameter Estimates\" table.    The AIC and BIC are going to be compared between different structures to determine the best.    The Null Model Likelihood Ratio Test tests the hypothesis versus So since this is VC vs. VC, this table contributes nothing.    The Fixed Effects table indicates that there is significant interaction, so we proceed normally.   For AR(1), we have   In the \"Covariance Parameter Estimates\" table, the AR(1) line indicates , and the Residual indicates . In particular, we see that . Similarly,     Comparing the AIC and BIC values with those from VC (215.6 vs. 245.4 and 216.6 vs. 245.9), since we go with smaller values, this suggests that AR(1) is more appropriate.    The NMLR test concludes that , so between the AIC\/BIC measurements and the NMLR test, we choose AR(1) as the appropriate structure.   For CS, we have   Since CS assumes covariance is the same, we see this in the R Matrix and the Parameter Estimates tables. Similarly, we see that .    We see that AR(1) has smaller AIC and BIC values than CS, so we prefer AR(1) to CS, and both over VC.    NMLR concludes that CS is more appropriate than VC.       For factorial treatment structure, we can still use similar code with a slight change:  DATA two;  INPUT A B time expunit response;  DATALINES;  ...  PROC MIXED DATA = two;  CLASS A B time expunit;  MODEL response = A | B | time \/ DDFM = SATTERTH  REPEATED \/ SUBJECT = expunit(A*B) TYPE = < option >     "
},
{
  "id": "p-449",
  "level": "2",
  "url": "lec-23-sec-1.html#p-449",
  "type": "Paragraph (with a defined term)",
  "number": "",
  "title": "",
  "body": "experiment with repeated measures "
},
{
  "id": "p-460",
  "level": "2",
  "url": "lec-23-sec-1.html#p-460",
  "type": "Paragraph (with a defined term)",
  "number": "",
  "title": "",
  "body": "covariance "
},
{
  "id": "example-17",
  "level": "2",
  "url": "lec-23-sec-1.html#example-17",
  "type": "Example",
  "number": "15.1.1",
  "title": "",
  "body": "  An experiment is conducted to determine the effect of salt in the soil on the emergence of grass seeds. There are three replications of four plots, each treated to contain 0, 8, 16, and 24% salt. The percent of seeds emerging is observed 5, 8, 11, and 14 days after planting. THere are 12 separate plots; that is, the three replications within each level of salt are unrelated. Within each plot, there are four repeated measurements corresponding to days after planting.   The repeated measures can be analyzed using the following SAS code:  DATA one;  INPUT trt time expunit response;  DATALINES;  ...  PROC MIXED DATA = one PLOTS=RESIDUALPANEL;  CLASS trt time expunit;  MODEL response = trt time trt*time\/ DDFM = SATTERTH  REPEATED \/ SUBJECT = expunit(trt) TYPE = < option >  The line SUBJECT = expunit(trt) shows that each experimental unit is nested within treatment. The TYPE = < option > specifies covariance structure.  Based on the SAS output (salt_output(1).pdf), we see the following (for VC):   The \"Estimated R Matrix\" identifies the relationship between the four time roles; this indicates that ; this is confirmed in the \"Covariance Parameter Estimates\" table.    The AIC and BIC are going to be compared between different structures to determine the best.    The Null Model Likelihood Ratio Test tests the hypothesis versus So since this is VC vs. VC, this table contributes nothing.    The Fixed Effects table indicates that there is significant interaction, so we proceed normally.   For AR(1), we have   In the \"Covariance Parameter Estimates\" table, the AR(1) line indicates , and the Residual indicates . In particular, we see that . Similarly,     Comparing the AIC and BIC values with those from VC (215.6 vs. 245.4 and 216.6 vs. 245.9), since we go with smaller values, this suggests that AR(1) is more appropriate.    The NMLR test concludes that , so between the AIC\/BIC measurements and the NMLR test, we choose AR(1) as the appropriate structure.   For CS, we have   Since CS assumes covariance is the same, we see this in the R Matrix and the Parameter Estimates tables. Similarly, we see that .    We see that AR(1) has smaller AIC and BIC values than CS, so we prefer AR(1) to CS, and both over VC.    NMLR concludes that CS is more appropriate than VC.      "
},
{
  "id": "lec-24-sec-1",
  "level": "1",
  "url": "lec-24-sec-1.html",
  "type": "Section",
  "number": "16.1",
  "title": "Analysis of Covariance",
  "body": " Analysis of Covariance   Motivation  With one-way ANOVA in a CRD, we look at a comparison of a single treatment with levels; we implicitly assume that we have homogeneous experimental units. However, often there are additional sources of variability which impact the response. In RCBD, we require a correct identification of blocking factor; what if we have a continuous blocking factor instead? RCBD requires a discrete factor, so you would have to introduce a level of subjectivity.  Suppose that in addition to a response , we observe some auxiliary variable . We call  covariate and consider it to be lienarly related to ; covariation is difficult to control in experimentation.  ANCOVA makes an adjustment of to remove the additional source of variability by that would be associated with . It involves fitting several linear lines to observations at each treatment level to account for the effect of the covariate and often leads to more precise statistical inference of treatment effects.    The effects of a slow-release fertilizer (S) and a fast-release fertilizer (F) on seed yield of peanut plants were compared to a control (C), which is a standard fertilizer, with 10 replications of each treatment. The researcher recognized that the 30 plants were not at the same level of development thus their heights were recorded at the start of the experiment.    Control (C)  Slow Release (S)  Fast Release (F)    Yield  Height  Yield  Height  Yield  Height    12.2  45  16.6  63  9.5  52    12.4  52  15.8  50  9.5  54    11.9  42  16.5  63  9.6  58    11.3  35  15  33  8.8  45    11.8  40  15.4  38  9.5  57    12.1  48  15.6  45  9.8  62    13.1  60  15.8  50  9.1  52    12.7  61  15.8  48  10.3  67    12.4  50  16  50  9.5  55    11.4  33  15.8  49  8.5  40    The results are plotted below.   Slow release (S) appears to show a higher yield than the other two. The yield also tends to depend on the initial height of the plant; we need to take the covariate into account for more accurate estimates of treatment effects.      Using ANCOVA  We can consider the following two models with a covariate in addition to the ANOVA model for a one-way treatment structure in a CRD:   Parallel Lines Model : for , , where    denotes response for the th replicate of treatment level      : the intercept for treatment level      : the slope (same for all treatment levels)     : covariate for th replicate of treatment level      : random error associated with such that     Non-Parallel Lines Model : for , , where    denotes response for the th replicate of treatment level      : the intercept for treatment level      : the slope for treatment level      : covariate for th replicate of treatment level      : random error associated with such that      Step-by-step analysis goes as follows:   Is the covariate necessary? Test vs. If we fail to reject , we proceed with one-way ANOVA. If we reject , proceed to step 2.    Are the lines parallel? Test vs. If we fail to reject , we adopt the parallel lines model. Then, each treatment effect can be compared at any value of the covariate. This is equivalent to testing vs. If we reject , we adopt the non-parallel lines model. Then, each treatment effect needs to be compared at different values of the covariates, which is equivalent to testing vs. where is some specific value of .       Consider the plant yield data from earlier:    Control (C)  Slow Release (S)  Fast Release (F)    Yield  Height  Yield  Height  Yield  Height    12.2  45  16.6  63  9.5  52    12.4  52  15.8  50  9.5  54    11.9  42  16.5  63  9.6  58    11.3  35  15  33  8.8  45    11.8  40  15.4  38  9.5  57    12.1  48  15.6  45  9.8  62    13.1  60  15.8  50  9.1  52    12.7  61  15.8  48  10.3  67    12.4  50  16  50  9.5  55    11.4  33  15.8  49  8.5  40    The SAS code is given by  DATA one;  INPUT trt $ height yield;  DATALINES;  ...  PROC GLM DATA = one;  CLASS trt;  MODEL yield = trt trt*height;  This gives the following table:   We reject ( ) at the significance level. We conclude that height is a significant covariate which as an effect on yield in at least one treatment level.  To test , add MODEL yield = trt height trt*height to the SAS code. This gives the following table:   The value on the last row tests We fail to reject under , so a parallel lines model is most appropriate.  Running the statement MODEL yield = trt height\/NOINT SOLUTION gives the table   This gives slope and intercept estimates for each line:   Treatment C:     Treatment F:     Treatment S:    The plot of the parallel lines against the response is given below:   To test which pair of treatment shows significant difference, use LSMEANS trt \/ ADJUST=Tukey and get   Note that each LSMEAN is calculated from each line at      "
},
{
  "id": "p-487",
  "level": "2",
  "url": "lec-24-sec-1.html#p-487",
  "type": "Paragraph (with a defined term)",
  "number": "",
  "title": "",
  "body": "covariate "
},
{
  "id": "example-18",
  "level": "2",
  "url": "lec-24-sec-1.html#example-18",
  "type": "Example",
  "number": "16.1.1",
  "title": "",
  "body": "  The effects of a slow-release fertilizer (S) and a fast-release fertilizer (F) on seed yield of peanut plants were compared to a control (C), which is a standard fertilizer, with 10 replications of each treatment. The researcher recognized that the 30 plants were not at the same level of development thus their heights were recorded at the start of the experiment.    Control (C)  Slow Release (S)  Fast Release (F)    Yield  Height  Yield  Height  Yield  Height    12.2  45  16.6  63  9.5  52    12.4  52  15.8  50  9.5  54    11.9  42  16.5  63  9.6  58    11.3  35  15  33  8.8  45    11.8  40  15.4  38  9.5  57    12.1  48  15.6  45  9.8  62    13.1  60  15.8  50  9.1  52    12.7  61  15.8  48  10.3  67    12.4  50  16  50  9.5  55    11.4  33  15.8  49  8.5  40    The results are plotted below.   Slow release (S) appears to show a higher yield than the other two. The yield also tends to depend on the initial height of the plant; we need to take the covariate into account for more accurate estimates of treatment effects.   "
},
{
  "id": "example-19",
  "level": "2",
  "url": "lec-24-sec-1.html#example-19",
  "type": "Example",
  "number": "16.1.2",
  "title": "",
  "body": "  Consider the plant yield data from earlier:    Control (C)  Slow Release (S)  Fast Release (F)    Yield  Height  Yield  Height  Yield  Height    12.2  45  16.6  63  9.5  52    12.4  52  15.8  50  9.5  54    11.9  42  16.5  63  9.6  58    11.3  35  15  33  8.8  45    11.8  40  15.4  38  9.5  57    12.1  48  15.6  45  9.8  62    13.1  60  15.8  50  9.1  52    12.7  61  15.8  48  10.3  67    12.4  50  16  50  9.5  55    11.4  33  15.8  49  8.5  40    The SAS code is given by  DATA one;  INPUT trt $ height yield;  DATALINES;  ...  PROC GLM DATA = one;  CLASS trt;  MODEL yield = trt trt*height;  This gives the following table:   We reject ( ) at the significance level. We conclude that height is a significant covariate which as an effect on yield in at least one treatment level.  To test , add MODEL yield = trt height trt*height to the SAS code. This gives the following table:   The value on the last row tests We fail to reject under , so a parallel lines model is most appropriate.  Running the statement MODEL yield = trt height\/NOINT SOLUTION gives the table   This gives slope and intercept estimates for each line:   Treatment C:     Treatment F:     Treatment S:    The plot of the parallel lines against the response is given below:   To test which pair of treatment shows significant difference, use LSMEANS trt \/ ADJUST=Tukey and get   Note that each LSMEAN is calculated from each line at    "
}
]
